...
resource "aws_lambda_function" "authorizer" {
  ...
  function_name     = "${var.prefix}-webhook-authorizer"
  ...
  environment {
    variables = {
      ...
      SOURCE_IP_ALLOW_LIST                     = jsonencode(var.source_ip_allow_list)
...
resource "aws_lambda_permission" "authorizer" {
  ...
  source_arn    = "${aws_apigatewayv2_api.webhook.execution_arn}/authorizers/${aws_apigatewayv2_authorizer.webhook.id}"
}
...
resource "aws_iam_role" "authorizer" {
  name                 = "${var.prefix}-action-webhook-authorizer-role"
...
resource "aws_iam_role_policy" "authorizer_logging" {
  name = "${var.prefix}-lambda-logging-policy"
  role = aws_iam_role.authorizer.name
  policy = templatefile("${path.module}/policies/lambda-cloudwatch.json", {
    log_group_arn = aws_cloudwatch_log_group.authorizer.arn
  })
}

resource "aws_iam_role_policy" "authorizer_ssm" {
  name = "${var.prefix}-lambda-ssm-policy"
  role = aws_iam_role.authorizer.name
